...
class migracion_Model extends CI_Model  {
	...
	public function Denuncias($start, $end, $location) {
  	$sq  = "	SELECT intentos, motivo_migracion, coyote_guia, lugar_de_usa, viaja_solo, deportado ";
		...
		$sq .= " , derechos.todos AS derechos, violaciones.todas as violaciones_derechos, autoridad_resp.nombre autoridad_responsable ";
		...
		$sq .= " 	 SELECT id_denuncia, GROUP_CONCAT(DISTINCT d.nombre ORDER BY d.nombre ASC SEPARATOR ' - ') AS todos ";
		...
		$sq .= " LEFT JOIN ( "; 
		//$sq .= " 	 SELECT id_denuncia, GROUP_CONCAT(v.nombre SEPARATOR ' - ') AS todas ";
		$sq .= " 	 SELECT id_denuncia, GROUP_CONCAT(DISTINCT v.nombre ORDER BY v.nombre SEPARATOR ' - ') AS todas ";
